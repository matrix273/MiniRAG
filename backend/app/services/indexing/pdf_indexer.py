...
async def find_toc_pages_async(start_page_index, page_list, opt, logger=None):
    """异步并发版本的 TOC 页面查找 - 性能优化"""
    print('start find_toc_pages (async concurrent)')
    
    # 确定需要检测的页面范围
    max_pages_to_check = min(len(page_list) - start_page_index, opt.toc_check_page_num)
    ...
    tasks = []
    page_indices = []
    for i in range(start_page_index, start_page_index + max_pages_to_check):
        tasks.append(toc_detector_single_page_async(page_list[i][0], model=opt.model))
    ...
    for i, (page_idx, result) in enumerate(zip(page_indices, results)):
        if isinstance(result, Exception):
            if logger:
                logger.error(f'Error checking page {page_idx}: {result}')
            if last_page_is_yes:
                break
            continue
        ...
        if result == 'yes':
            ...
            if logger:
                logger.info(f'Found the last page with toc: {page_idx - 1}')
            break
    
    if not toc_page_list and logger:
        logger.info('No toc found')
        
    print(f'find_toc_pages completed: found {len(toc_page_list)} TOC pages')
    return toc_page_list
...
async def check_toc_async(page_list, opt=None):
    """异步并发版本的 TOC 检查 - 性能优化"""
    print('start check_toc (async concurrent)')
    ...
    print(f'toc found: {len(toc_page_list)} pages')
    toc_json = toc_extractor(page_list, toc_page_list, opt.model)

    if toc_json['page_index_given_in_toc'] == 'yes':
        print('index found in initial TOC pages')
        return {'toc_content': toc_json['toc_content'], 'toc_page_list': toc_page_list, 'page_index_given_in_toc': 'yes'}
    ...
    while (toc_json['page_index_given_in_toc'] == 'no' and 
        ...
        additional_toc_json = toc_extractor(page_list, additional_toc_pages, opt.model)
        if additional_toc_json['page_index_given_in_toc'] == 'yes':
            print('index found in additional TOC pages')
            return {'toc_content': additional_toc_json['toc_content'], 'toc_page_list': additional_toc_pages, 'page_index_given_in_toc': 'yes'}
