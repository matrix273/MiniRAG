...
class ConfigLoader:
    ...
    DEFAULT_CONFIG = {
        ...
        "retrieve_model": "qwen/qwen-turbo",
    ...
    def _load_config_from_env(self):
        """从环境变量加载配置"""
        config_dict = dict(self.DEFAULT_CONFIG)
        
        for env_var, config_key in self.ENV_MAPPING.items():
            env_value = os.environ.get(env_var)
            if env_value is not None:
                # 转换类型
                default_value = self.DEFAULT_CONFIG[config_key]
    ...
    def _validate_keys(self, user_dict):
        unknown_keys = set(user_dict) - set(self.DEFAULT_CONFIG)
        if unknown_keys:
            raise ValueError(f"Unknown config keys: {unknown_keys}")

    def load(self, user_opt=None) -> config:
        """
        加载配置，合并环境变量、默认值和用户选项。
        优先级：用户选项 > 环境变量 > 默认值
        """
        if user_opt is None:
            user_dict = {}
